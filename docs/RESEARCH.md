...
| QuillBot Paraphraser modes | 유창성 모드는 문법과 자연스러움에 집중하며 변경을 최소화 | `fluent` 최소 수정 모드에 원리만 반영 |
| Grammarly tone suggestions | 더 친근하고 긍정적이며 자신감 있게 들리도록 문장별 말투 제안을 제공 | 독자가 받는 인상 점검을 뜻 보존 범위로 제한 |
| Wordtune Rewrite controls | 격식/비격식 및 길이 조절을 분리 | 높임 정도와 `concise`를 분리하고 내용 확장은 제외 |
