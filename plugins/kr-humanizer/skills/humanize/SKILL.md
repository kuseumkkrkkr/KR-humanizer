...
2. When the user starts from a writing prompt rather than a draft, ask whether to use Plan mode. If accepted, run `npx --yes github:kuseumkkrkkr/KR-humanizer plan <brief> --explanation balanced --out <graph.json>`, present the nodes before prose, and let the user edit, reorder, exclude, or delete nodes. In the GUI, the selected Codex model (including Spark) is used for both Plan and graph drafting. Then run `draft <brief> --graph <graph.json>`. Never restore an excluded node in the draft.
...
6. For a visual review, run `npx --yes github:kuseumkkrkkr/KR-humanizer gui`. Tell the user it opens a localhost interface.
   - If the user wants next-sentence assistance and Codex EXEC is available, enable `Tab 문장 자동완성` in the GUI. It waits for a pause at the end of the draft, lets the user choose an allowlisted Codex model (default `gpt-5.3-codex-spark`), shows one sentence before insertion, accepts with Tab or the Apply button, and dismisses with Escape. Never imply that this mode works through Claude Code or a separate API.
7. For a CLI proposal, run `npx --yes github:kuseumkkrkkr/KR-humanizer rewrite <file> --engine codex --mode medium --honorific 50 --explanation balanced --out <proposal.json>` in Codex, or use `--engine claude` in Claude Code. Add `--graph <graph.json>` when the user has reviewed a graph. The rewrite command searches the vault and injects only the top matching guidance into the prompt. Use `--vault <folder>` for a user-maintained Obsidian vault. Use exactly one of three modes: `weak` changes tone/endings only; `medium` also checks the meaning graph, formulaic AI phrasing, re-explanation, repetition, and over-explanation; `strict` adds NIKL norms and detailed grammar, agreement, modifier-scope, and ambiguity review. Read [editing principles](references/editing-principles.md) when choosing or explaining a mode.
...
- Next-sentence autocomplete is Codex EXEC-only. The GUI may select only the allowlisted models and must validate the value server-side; do not silently accept a suggestion.
