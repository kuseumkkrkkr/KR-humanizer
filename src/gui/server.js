  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === '/api/rewrite') {
        ...
        const rewritten = await rewriteWithEngine({ engine: body.engine ?? 'codex', text: body.text ?? '', brief: briefValue(body.brief), contextGraph: body.contextGraph, tone: body.tone, editMode: body.editMode, honorificLevel: body.honorificLevel, explanationLevel: body.explanationLevel, memories, model: body.model });
